...
class Detector(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.net.load_state_dict(torch.load('models/net.pth'))
    ...
    def _filter(self, output, thresh):
        ...
        mask = output[..., 4] > thresh  #得到置信度大于阈值的掩码
        print('mask', mask)
        idxs = mask.nonzero()   #得到置信度大于阈值的索引
        vecs = output[mask]
        # print(vecs[..., 4])
        return idxs, vecs
...
if __name__ == '__main__':
    detetor = Detector()
    y = detetor(torch.randn(3, 3, 416, 416), 0.2, cfg.ANCHORS_GROUP)
    print(y)
